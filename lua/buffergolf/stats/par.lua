...
local function typing_mode_par(reference_lines)
	if not reference_lines or #reference_lines == 0 then return 0 end
...
local function golf_par_from_hunks(session)
	...
	if not practice_lines or not reference_lines then return nil end

	local ok, minidiff = pcall(require, 'mini.diff')
	if not ok then return nil end

	local buf_data = minidiff.get_buf_data(session.reference_buf)
	if not buf_data or not buf_data.hunks or #buf_data.hunks == 0 then return 0 end
