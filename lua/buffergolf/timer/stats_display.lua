...
function M.create_stats_window(session)
  ...
  local buf_opts =
    { bufhidden = "wipe", buftype = "nofile", modifiable = false, swapfile = false, filetype = "BuffergolfStats" }
  ...
  local win_opts = {
    ...
    wrap = false,
    cursorline = false,
    winhl = "Normal:BuffergolfStatsFloat",
    statusline = " ",
    winbar = "",
  }
...
function M.render_stats(session, time_str, wpm, keystrokes, par)
  ...
  if not buffer.buf_valid(stats_buf) or not buffer.win_valid(stats_win) then
    return
  end
