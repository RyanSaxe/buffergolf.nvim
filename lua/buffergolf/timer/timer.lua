...
local function get_elapsed_seconds(session)
	if not session.timer_state or not session.timer_state.start_time then return 0 end
	return math.floor((vim.loop.hrtime() - session.timer_state.start_time) / 1e9)
end

local function get_display_time(session)
	if not session.timer_state.start_time then return "--:--" end
...
local function check_completion(session)
	if not buffer.buf_valid(session.practice_buf) or not session.reference_lines then return false end
	local ok, actual_lines = pcall(vim.api.nvim_buf_get_lines, session.practice_buf, 0, -1, true)
	if not ok or not actual_lines then return false end
	...
	if #actual_lines ~= #reference_lines then return false end
	for i = 1, #actual_lines do
		if actual_lines[i] ~= reference_lines[i] then return false end
	end
	return true
end
...
local function complete_session(session, reason)
	if session.timer_state.completed then return end
...
local function check_countdown_expired(session)
	if not session.timer_state.countdown_mode or session.timer_state.completed then return false end
...
function M.update_stats_float(session)
	...
	if not ts.stats_buf or not buffer.buf_valid(ts.stats_buf) or
	   not ts.stats_win or not buffer.win_valid(ts.stats_win) then
	...
	if not buffer.win_valid(session.practice_win) then return end
...
function M.on_first_edit(session)
	if not session.timer_state or session.timer_state.start_time then return end
	session.timer_state.start_time = vim.loop.hrtime()
	M.update_stats_float(session)
end

function M.start_countdown(session, seconds)
	if not session or not session.timer_state then return end
...
function M.init(session)
	session.timer_state = {
		start_time = nil, countdown_mode = false, countdown_duration = 0,
		locked = false, completed = false, frozen_time = nil,
		frozen_wpm = nil, frozen_keystrokes = nil,
		stats_win = nil, stats_buf = nil, update_timer = nil,
	}
	...
	timer:start(250, 250, vim.schedule_wrap(function()
		if not session.timer_state or not buffer.win_valid(session.practice_win)
		   or not buffer.buf_valid(session.practice_buf) then
			timer:stop()
			timer:close()
			return
		end
