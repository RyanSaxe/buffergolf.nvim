...
function M.goto_hunk_sync(session, direction)
	if not session or not session.reference_buf or not session.practice_buf then return end
		...
		if buffer.win_valid(ref_win) then
			...
			if goto_ok then
				...
				if buf_data and buf_data.hunks then
					...
					if line_offset ~= 0 then practice_line = ref_line + line_offset end
				end
...
function M.setup(session)
	...
	local defaults = {
		next_hunk = "]h", prev_hunk = "[h", first_hunk = "[H",
		last_hunk = "]H", toggle_overlay = "<leader>do"
	}
