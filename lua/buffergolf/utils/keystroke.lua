...
function M.init_session(session)
	if not session or not session.practice_buf then return end
	...
	vim.on_key(function(raw_key)
		...
		if not ok or current_buf ~= practice_buf then return end

		local state = active_sessions[practice_buf]
		if not state or vim.fn.reg_executing() ~= "" then return end

		local key_name = vim.fn.keytrans(raw_key)
		if key_name == "<Ignore>" or key_name:match("^<.*Mouse.*>$") or
		   raw_key == "" or key_name == "" then return end
		...
		if command_depth == 0 then
			...
			command_depth = 1
			if depth_reset_timer then vim.loop.timer_stop(depth_reset_timer) end
...
function M.get_count(session)
	if not session or not session.practice_buf then return 0 end
...
function M.set_tracking_enabled(session, enabled)
	if not session or not session.practice_buf then return end
	local state = active_sessions[session.practice_buf]
	if state then state.tracking_enabled = enabled end
end

function M.is_tracking_enabled(session)
	if not session or not session.practice_buf then return false end
	local state = active_sessions[session.practice_buf]
...
function M.with_keys_disabled(session, fn)
	...
	local ok, result = pcall(fn)
	if was_enabled then M.set_tracking_enabled(session, true) end
	if not ok then error(result) end
	return result
end
