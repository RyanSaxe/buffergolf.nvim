...
local function ensure_line_count(session)
	...
	if not buffer.buf_valid(bufnr) then return end
	local needed, current = math.max(#session.reference_lines, 1), vim.api.nvim_buf_line_count(bufnr)
	if current < needed then
		local extra = {}
		for _ = current + 1, needed do table.insert(extra, "") end
...
local function expand_tabs(text, actual, bufnr)
	...
	local tabstop = pcall(vim.api.nvim_get_option_value, "tabstop", { buf = bufnr }) and
		vim.api.nvim_get_option_value("tabstop", { buf = bufnr }) or 8
...
function M.set_ghost_mark(session, row, col, text, actual)
	if text == "" then
		if session.ghost_marks[row] then M.clear_ghost_mark(session, row) end
		return
	end
	...
	session.ghost_marks[row] = vim.api.nvim_buf_set_extmark(
		session.practice_buf, session.ns_ghost, row - 1, col, opts)
end

function M.refresh(session)
	if session.refreshing then return end
	session.refreshing = true

	local ok = pcall(function()
		if not buffer.buf_valid(session.practice_buf) then return end

		if session.mode == "golf" then
			if session.update_mini_diff then session.update_mini_diff() end
			return
		end
		...
		for row = 1, total do
			...
			for i = 1, math.min(#actual, #reference) do
				if actual:sub(i, i) ~= reference:sub(i, i) then break end
				prefix = i
			end
			...
			if not prev_range or prev_range.start ~= mismatch_start or prev_range.finish ~= mismatch_finish then
				...
				if mismatch_finish > mismatch_start then
					vim.api.nvim_buf_add_highlight(bufnr, session.ns_mismatch,
						session.config.mismatch_hl, row - 1, mismatch_start, mismatch_finish)
				end
		...
		for row in pairs(session.ghost_marks) do
			if row > total then M.clear_ghost_mark(session, row) end
		end
		if session.mismatch_ranges then
			for row in pairs(session.mismatch_ranges) do
				if row > total then session.mismatch_ranges[row] = nil end
			end
		end
	...
	if not ok then vim.notify("Buffergolf refresh error", vim.log.levels.ERROR) end
end

function M.attach_change_watcher(session, opts)
	if session.change_attached or not buffer.buf_valid(session.practice_buf) then return end
	opts = opts or {}

	local ok, res = pcall(vim.api.nvim_buf_attach, session.practice_buf, false, {
		on_lines = function(_, buf)
			if session.refreshing or not buffer.buf_valid(buf) then return end
			if opts.is_session_active and not opts.is_session_active(buf, session) then return true end
			if opts.on_first_edit then opts.on_first_edit(session) end
			if opts.schedule_refresh then opts.schedule_refresh(session) end
		end,
		on_detach = function() session.change_attached = nil end,
	})

	if ok and res then session.change_attached = true end
end
