...
function M.schedule_refresh(session)
  ...
  vim.schedule(function()
    ...
    if
      not session
      or not session.practice_buf
      or storage.by_practice(session.practice_buf) ~= session
      or not buffer.buf_valid(session.practice_buf)
      or (session.timer_state and (session.timer_state.locked or session.timer_state.completed))
    then
      return
    end
