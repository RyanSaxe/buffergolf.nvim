	...
	if session.mode == "golf" and session.minidiff_enabled then
		...
		if ok then
			for _, buf in ipairs({session.reference_buf, session.practice_buf}) do
				if buffer.buf_valid(buf) then pcall(minidiff.disable, buf) end
			end
		end
	end
...
local function setup_practice_buffer(practice_buf, origin_buf, origin_ft)
	...
	local vars = {
		buffergolf_practice = true, copilot_enabled = false,
		copilot_suggestion_auto_trigger = false, autopairs_enabled = false,
		minipairs_disable = true, buffergolf_active = true
	}
...
function M.start(origin_bufnr, config, target_lines)
	if storage.is_active(origin_bufnr) then return end

	local reference = target_lines or vim.api.nvim_buf_get_lines(origin_bufnr, 0, -1, true)
	if #reference == 0 then reference = { "" } end
	...
	local empty_lines = {}
	for _ = 1, math.max(1, #reference) do table.insert(empty_lines, "") end
...
function M.reset_to_start(bufnr)
	...
	if session.mode == "typing" then
		local empty_lines = {}
		for _ = 1, math.max(1, #session.reference_lines) do table.insert(empty_lines, "") end
