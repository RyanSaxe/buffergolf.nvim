...
function M.copy_indent_options(origin, target)
	local opts = {"expandtab", "tabstop", "softtabstop", "shiftwidth", "autoindent",
		"smartindent", "cindent", "indentexpr", "copyindent", "preserveindent"}
	for _, opt in ipairs(opts) do
		local ok, value = pcall(vim.api.nvim_get_option_value, opt, { buf = origin })
		if ok then pcall(vim.api.nvim_set_option_value, opt, value, { buf = target }) end
	end
end
...
function M.generate_buffer_name(origin_bufnr, suffix)
	...
	if vim.fn.filereadable(name) == 1 then
		vim.notify("WARNING: Found existing file '" .. name .. "' on disk. " ..
			"BufferGolf practice/reference buffers should be temporary and never exist as real files. " ..
			"Please remove or rename this file.", vim.log.levels.ERROR, { title = "buffergolf" })
	end
	return name
end
...
function M.normalize_lines(lines, bufnr)
	...
	if not ok or not expandtab then return lines end
...
function M.strip_trailing_empty_lines(lines)
	...
	if last_non_empty == 0 then return {} end
	local result = {}
	for i = 1, last_non_empty do result[#result + 1] = lines[i] end
	return result
end
...
function M.disable_matchparen(session)
	if session.config.disable_matchparen == false then return end

	local win = session.practice_win
	if not M.win_valid(win) then return end
	...
	value = value:match("MatchParen:") and value:gsub("MatchParen:[^,%s]+", "MatchParen:None") or
		(value ~= "" and value .. ",MatchParen:None" or "MatchParen:None")
