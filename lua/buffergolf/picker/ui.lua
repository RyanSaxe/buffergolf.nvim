...
local function show_start_state_picker(origin_buf, target_lines, is_selection, config)
  ...
  vim.ui.select(options, {
    ...
    local sources = {
      ...
      file = function()
        file_source.select(origin_buf, target_lines, config, start_golf_fn)
      end,
      buffer = function()
        buffer_source.select(origin_buf, target_lines, config, start_golf_fn)
      end,
      register = function()
        register_source.select(origin_buf, target_lines, config, start_golf_fn)
      end,
      git = function()
        git_source.select(origin_buf, target_lines, config, start_golf_fn)
      end,
    }
    if sources[choice.value] then
